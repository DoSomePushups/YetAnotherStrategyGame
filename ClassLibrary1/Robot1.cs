    ...
    public class Robot1 : IRobot
    {
        ...
        public void MakeMove()
        {
            ...
            foreach (var unit in allMyUnits)
            {
                ...
                else if (unitMemory.Role == Role.Military)
                {
                    ...
                    else if (unit.HP < unit.GetMaxHP() && !isDireEmergency && !IsAttackingPhase || unit.HP < unit.GetMaxHP() / 2 && !isDireEmergency)
        ...
        private void ExecuteIntent(IUnit controlledUnit, UnitMemory unitMemory, List<IEntity> enemyEntities)
        {
            switch (unitMemory.Intent)
            {
                case Intent.Heal:
                case Intent.Rally:
                    if (controlledUnit is IRangedUnit rangedUnit)
                        CheckSurroundings(rangedUnit);
                ...
                case Intent.Defend:
                    ...
                    if (controlledUnit is Cannon)
                    {
                        var enemyBuildings = targets.Where(enemy => enemy is IBuilding building 
                        && building.Location.GetDistance(controlledUnit.Location) <= CannonInformation.Range).ToList();
